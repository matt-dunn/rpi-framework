		...
		<xsl:when test="string-length(normalize-space(@href)) &gt; 0 or string-length(normalize-space(@name)) &gt; 0">
			<a>
				<xsl:if test="string-length(normalize-space(@href)) &gt; 0">
					<xsl:variable name="isUrlExternal"><xsl:apply-templates select="@href" mode="ext_isUrlExternal"/></xsl:variable>
					<xsl:attribute name="href"><xsl:apply-templates select="@href" mode="xhtml_fragment_anchor"/></xsl:attribute>
...
<!-- Solr sends hightlights through as <em/> tags -->
<xsl:template match="xhtml:em" mode="xhtml_fragment_search">
	<xsl:param name="headingLevel" select="number(1)"/>
